...
public class UpdateImageActivity extends BaseActivity {
    ...
    @OnClick({R.id.imageshow, R.id.selectimage, R.id.left})
    ...
    private void updateimage(String imagename, String file) {
        RetrofitHttpUtil.getApiService()
                .UploadFile("", SPUtils.getUserId(context), 3, SPUtils.getShopId(context), 1, imagename,true, file)
                .compose(SchedulerTransformer.<String>transformer())
                ...
                .subscribe(new BaseObserver<String>() {
                    ...
                    protected void onSuccess(String s) {
                        if (s.contains("1")) {
                            showToast("上传图片成功");
                            showstoreimage();
                            EventBusUtil.sendStickyEvent(new MessageEvent(MyEventCode.CODE_B, "UpdateImageInfo"));
                        }
                        LogUtils.d("图片" + s);
                    }
